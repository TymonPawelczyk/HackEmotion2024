...
if not os.path.exists(pic_dir):
    os.makedirs(pic_dir)
labels = ["Sad", "Happy", "Surprise", "Angry", "Neutral", "Disgusted", "Scared"]
for label in labels:
...
while True:
    ...
    if anyFaces:
        ...
        cv2.putText(frame, "Press (h)Happy, c(Scared), (i)Disgust, (a)Angry", (12,15), cv2.QT_FONT_NORMAL, 0.5,font_color , 1)
        cv2.rectangle(frame, (0, 20), (640, 40), back_color, -1)
        cv2.putText(frame, "(s)Surprise, (n)Neutral, (d)Sad, (q)uit", (12,35), cv2.QT_FONT_NORMAL, 0.5,font_color , 1)
    ...
    if key == ord('c') and f_im is not None:
        save_file("Scared", f_im)
    
    if key == ord('i') and f_im is not None:
        save_file("Disgusted", f_im)
